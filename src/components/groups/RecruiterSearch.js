...
const StudentSearch = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await axios.get('http://18.183.141.57/management/recruiter-list/');
      let fullNameList = [];
      let sem = [];
      let department = [];
      let descr = [];
      let datamap = [];
      let searchId = [];
      let typeAPI = "recruiter";
      res.data.map((data) => {
        fullNameList.push(data.name);
        sem.push(data.position);
        descr.push(data.looking_for);
        department.push(data.org_name);
