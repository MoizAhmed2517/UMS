...
const icons = [
    {
        ...
        path: '/Academic-Records-Search'
    },
    {
        icon: <AssessmentIcon />,
        tooltip: 'Technical Records',
        styles: {fontSize: 30, filter: 'drop-shadow(1px 0px 5px rgba(0, 0, 0, 0.5))'},
        path: '/Technical-Records-Search'
    },
]
                        ...
                        <Grid item xs={6} marginTop={1} key={index} marginBottom={1}>
                            <Stack direction='column'>
                                <LightTooltip title={tooltip}>
                                    <IconButton size='large' color='primary' style={{ color: '#153E52', height: 50, width: 50, marginLeft: 54 }} component={Link} to={path} state={{ id: props.userId, displayStatus: props.displayStatus }}> 
